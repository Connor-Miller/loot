...
**Identity portability** — moving the *same* identity to another machine via `loot id export <file>` / `loot id import <file>` (ADR 0016). The exported file is always passphrase-wrapped (OpenSSH native encryption) because it travels and is the highest-risk artifact; the in-repo `.loot/id` stays unencrypted at rest (filesystem perms 0600). Distinct from *rotation* (a new key while staying the same identity to peers), shipped as `loot id rotate` (#16): new keypair + a re-grant wave reusing [[Grant]] rather than cryptographic key-succession — every still-live grant this identity holds is re-issued as a targeted bundle carrying its original `expires_at` exactly (never extending a lapsing grant, #20), the old key is archived at `.loot/id.rotated-<ts>` (never deleted — the emergency-rollback artifact), and peers re-trust the new pubkey manually via `loot peer add`.
...
**Grant discovery (`grants` / `pull-grants`)** — three honest verbs with distinct contracts so network I/O never creeps into offline commands. `loot status` stays strictly local (snapshot + report, no network). `loot grants` makes a deliberate network call to *peek* the relay mailbox — reports the pending count without draining ("2 grants pending at origin"). `loot pull-grants` is the mutating fetch: it drains the mailbox, verifies each grant's grantor signature, applies those from registered peers, and files the unsealed keys. This is pull-based discovery — the recipient must run `loot grants` to learn what is waiting; true push notification needs relay-initiated delivery infrastructure and is out of scope.

**Quarantine** *(#12)* — where `pull-grants` files a grant from a pubkey the [[Peer registry]] doesn't recognize (ADR 0015), rather than dropping it silently. Persisted at `.loot/quarantine/<sender pubkey hex>/<oid hex>` (shared-store-rooted, like the keyring — one identity's mailbox, not a lane concern; disjoint per-(sender, oid) filenames make concurrent quarantines lock-free, the same reasoning as loose object storage, ADR 0012), each entry holding the received-at timestamp plus the raw `Frame::SealedGrant` bundle bytes verbatim — including, since [[Grant expiry]] (#20), any `expires_at` the sender set: re-applying a quarantined grant runs through the same `apply_sealed_grant` gate, so a grant that has since expired is rejected on trust exactly as a fresh one would be, not given a free pass for having waited in quarantine. `loot grants --quarantined` lists every pending entry (sender pubkey hex, oid, received time) — purely local, no network. `loot grants --trust <pubkey-hex>` registers the sender as a peer (named by its own pubkey hex, since a bare key carries no nickname) and re-applies every grant of theirs still held, moving each one out of quarantine as it succeeds; a grant that fails to re-apply (including on expiry) stays quarantined rather than being dropped. Neither the review nor the trust step needs the sender to have ever been reachable again — everything required to finish the handoff already arrived with the original (quarantined) push.
