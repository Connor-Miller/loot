...
- loot change: `yxvnvtox` (version `b071df66`) "review fixes: retire with_repo, collapse forwarders, reconcile_* renames, fork-view naming" — the last of the day's ten loot-first lanes
- ferry: up to date (nothing to ingest or project)
- pushed: 0 object(s) to the relay
- git backup HEAD: `4d9b3c0` "review fixes: retire with_repo, collapse forwarders, reconcile_* renames, fork-view naming (#177/#178/#182 + code-review)"
- loot head at start of day: `650d88a`'s loot change `xsynprqr` (pre-land gate, PR #173) — by ritual time the head was already `yxvnvtox`, because the day ran loot-first
- friction: **the inversion day** — first drive day where loot *led*. All ten changes (pre-land gate #173, architecture wave #185–#191, review-fix #192) went `loot describe` → review PR → `loot-first.ps1 land` (finalize → ferry → git-main projection → relay push), so the end-of-day ritual found a clean tree, `ferry: up to date`, and **0 objects to push** — the ritual is now a verification pass, not the publish. Two nits, both in the helper not loot: `loot-day.ps1` parses `loot log`'s first line for the head, which since the columnar log (PR #159) is the *header row*, so it logged the change id as literally `change` (fixed in the helper this day, entry corrected by hand); and its message parameter is misleading on a loot-first day since the snapshot it would label never happens. One loot papercut: `loot log` exits 255 when its stdout pipe closes early (e.g. `Select-Object -First 1`), which reads as failure to shell tooling.